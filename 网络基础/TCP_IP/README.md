# TCP/IP协议
tcp/ip是一个协议族的统称，包括IP协议，IMCP协议，TCP协议，以及http、ftp、pop3协议等。
...
* 网络层:网络层用来处理在网络上流动的数据包。 数据包是网络传输的最小数据单位。 该层规定了通过怎样的路径（所谓的传输路线） 到达对方计算机， 并把数据包传送给对方。`IP协议就是在这里`
...
4、每一条TCP连接只能是`点到点的`;UDP支持`一对一，一对多，多对一和多对多`的交互通信
...
### 为什么TCP建立连接要三次握手？不是两次，五次？
首先得回答三次握手的`目的是同步连接双方的序列号和确认号并交换 TCP 窗口大小信息`。
...
具体问题具体解决，总结就是
* 在处理资源时一定要记住自己申请的资源要记得主动释放
* 查代码，问题出在服务器里面


参考 [再叙TIME_WAIT](http://huoding.com/2013/12/31/316)  
     [Time_wait与Close_wait](http://blog.csdn.net/shootyou/article/details/6622226)
